//! Fixed4 wrapper for `top_p` sampling values.

use super::sampling_fixed4::Fixed4;

/// `top_p` encoded in fixed4 (`10_000 => 1.0000`).
///
/// The encoded value `65_535` is reserved as a `None` sentinel in packed
/// optional fields and is therefore not a valid `TopPFixed4` payload.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
#[repr(transparent)]
pub struct TopPFixed4(Fixed4);

impl TopPFixed4 {
    /// Maximum encoded value representable by [`TopPFixed4`].
...
mod tests {
    ...
    fn sentinel_is_not_accepted() {
        assert!(TopPFixed4::from_encoded(Fixed4::NONE_SENTINEL).is_none());
    }

    #[test]
    fn value_roundtrips() {
        let value = TopPFixed4::from_encoded(5_000).expect("valid fixed4 value");
        assert_eq!(value.encoded(), 5_000);
        assert_eq!(value.value(), 0.5);
    }

    #[test]
    fn max_encoded_excludes_sentinel() {
        assert_eq!(TopPFixed4::MAX_ENCODED, Fixed4::NONE_SENTINEL - 1);
    }
}
