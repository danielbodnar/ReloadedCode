...
use llm_coding_tools_core::models::{
    Modality, ModelCatalogBuilder, ModelInfo, ProviderInfo, ProviderType,
};

struct Dataset {
    providers: Vec<(String, ProviderInfo)>,
...
fn benchmark_builder_construction(c: &mut Criterion) {
    ...
    for (name, provider_count, model_count) in [
        ("models_dev_snapshot", 96usize, 1669usize),
        ("max", 16384usize, 65535usize),
    ] {
        let dataset = make_dataset(provider_count, model_count);
        group.throughput(Throughput::Elements((provider_count + model_count) as u64));
